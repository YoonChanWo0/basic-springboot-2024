package com.cw98.backboard.controller;
    ...
    public String reset_mail(Model model, @RequestParam("email")String email) {
        ...
        try {
             ...
             if (result) {
                ...
                model.addAttribute("result", "초기화 매일 전송성공!");
             }else{
                model.addAttribute("result", "초기화 매일 전송실패! 관리자에게 문의하세요.");
             }
             
        } catch (Exception e) {
            model.addAttribute("result", "초기화 매일 전송실패! 사용자가 없습니다!");
        }
